...
def soluter(far):
    ...
    for q in range(len(n0)):
        q1=q+1
        n_imag_init=[] #记录没有增加虚增益前的初始有源区折射率虚部，用于减去求得漏模损耗
        ...
        if x_submax>3E-6:
            x_submax=3E-6
        E_structure=[] #记录没有考虑无限衬底时的光场，因为计算限制因子不能加衬底
        x_structure=[]
        for i in range(len(E)):
            E_structure.append(E[i])
            x_structure.append(x[i])
        ...
        if far==1: #是否绘制远场
            for i in range(5000): #计算与绘制远场，括号的数决定把衬底分成多少份
                x_structure.append(t[-1]+d_sub*1E-6*i/5000)
                E_structure.append(B[-1]*np.exp(-γ[-1]*d_sub*1E-6*i/5000)) #删掉了A项，否则会导致光场在x很大时发散
            θ=[] #计算远场
            Iθ=[]
            for i in range(200):
                θ.append(-50+0.5*i)
                integral_Iθ=0
                for j in range(len(x_structure)-1):
                    x2=float(x_structure[j+1])
                    x1=float(x_structure[j])
                    integral_Iθ=integral_Iθ+(x2-x1)*E_structure[j]*np.exp(k0*1j*np.sin(θ[i]*2*np.pi/360)*x1)
                Iθ.append(np.cos(θ[i]*2*np.pi/360)**2*abs(integral_Iθ)**2)
            plt.figure()
