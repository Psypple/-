...
iter_max2_reduce=100 #设置增益倍增后缩减增益的最大迭代次数
net_gain=1E-6 #计算结果净增益值上限
δ_gain_init=100*100*λ/(4*np.pi) #收敛后，迭代增益使得净增益为0的初始步长.这一步不当会导致杂模解！！！
δ_gain_min=1*100*λ/(4*np.pi) #最小步长
...
def soluter():
    ...
    for q in range(len(n0)):
        ...
        count=0
        noleak=converge_state #此值为1时跳过增益迭代
        while converge_state==0 and count<iter_max2: #不收敛时开始增加增益
            ...
            for i in active_index: #倍增增益
                n[i+1]=n[i+1].real+n[i+1].imag*1.1j
        if converge_state==0:
            print ('Failed to convert.')
        else:
            print ('Converting succeeds.')
            if noleak==0:
                for i in active_index: #把冗余的增益倍增还原
                    n[i+1]=n[i+1].real+n[i+1].imag/1.1*1j
                count=0
                crimedef=0
                while abs(neff_imag)>net_gain and count<iter_max2_reduce: #迭代增益使得净增益为0
                    for i in active_index:
                    ...
                    if converge_state==1:
                        if neff_imag<0:
                            if neff_imag<-1*net_gain: #这里假设了neff虚部的结果与有源区增益关系是单调的！！！
                                if crimedef==0:
                                    δ_gain=1.1*abs(δ_gain)
                                else:
                                    crimedef=0
                                    δ_gain=abs(δ_gain)
                        else:
                        ...
                        for i in active_index:
                            n[i+1]=n[i+1]-δ_gain*1j
                        if δ_gain>=δ_gain_min:
                            δ_gain=0.5*δ_gain
                        else:
                            print ('Failed to convert when reducing the gain.')
                            break
                        crimedef=1
                    count=count+1
        A=[]
