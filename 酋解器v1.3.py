...
def soluter():
    ...
    for q in range(len(n0)):
        ...
        if converge_state==0:
            ...
            if noleak==0:
                ...
                while count<iter_max2_reduce and abs(neff_imag)>net_gain: #downhill迭代增益使得净增益为0
                    ...
                    neff_imag_gain.append(ABγ[1].imag)
                    count=count+1
                if converge_state==0:
                    ...
                    print ()
                    print ('Gain-guide required.')
                    print ('neff=%f +% f i'%(ABγ[1].real,ABγ[1].imag))
                    gain_need=abs(ABγ[1].imag*(4*np.pi)/(100*λ))
                    print ('minima gain=%f /cm'%gain_need)
                    print ()
                    plt.figure()
        ...
        αi=abs(ABγ[1].imag)*(4*np.pi)/(100*λ)
        print ('Γ=%f' %Γ)
        print ('Modal absorption loss=%f'%αi)
