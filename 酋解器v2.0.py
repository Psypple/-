...
active_index_list1=table['是否为有源区1（是为1）'].values.tolist() #记录是否为有源区
active_index_list2=table['是否为有源区2（是为1）'].values.tolist() #记录是否为有源区
active_index_list3=table['是否为有源区3（是为1）'].values.tolist() #记录是否为有源区
active_index_list4=table['是否为有源区4（是为1）'].values.tolist() #记录是否为有源区
active_index_list5=table['是否为有源区5（是为1）'].values.tolist() #记录是否为有源区
active_index=[]
active_index_list=[]
for i in range(len(active_index_list1)):
    active_index_list.append(active_index_list1[i])
for i in range(len(active_index_list2)):
    active_index_list.append(active_index_list2[i])
for i in range(len(active_index_list3)):
    active_index_list.append(active_index_list3[i])
for i in range(len(active_index_list4)):
    active_index_list.append(active_index_list4[i])
for i in range(len(active_index_list5)):
    active_index_list.append(active_index_list5[i])
for i in range(len(active_index_list)):
    if active_index_list[i]==1:
        active_index.append(i)
active_index_list=[active_index_list1_ for active_index_list1_ in active_index_list if active_index_list1_==active_index_list1_]

'''输入折射率和厚度'''
n_real1=table['折射率实部1'].values.tolist()
n_real2=table['折射率实部2'].values.tolist()
n_real3=table['折射率实部3'].values.tolist()
n_real4=table['折射率实部4'].values.tolist()
n_real5=table['折射率实部5'].values.tolist()
n_real=[]
for i in range(len(n_real1)):
    n_real.append(n_real1[i])
for i in range(len(n_real2)):
    n_real.append(n_real2[i])
for i in range(len(n_real3)):
    n_real.append(n_real3[i])
for i in range(len(n_real4)):
    n_real.append(n_real4[i])
for i in range(len(n_real5)):
    n_real.append(n_real5[i])
n_real=[active_index_list1_ for active_index_list1_ in n_real if active_index_list1_==active_index_list1_]
n_imag1=table['吸收1（/cm）'].values.tolist()
n_imag2=table['吸收2（/cm）'].values.tolist()
n_imag3=table['吸收3（/cm）'].values.tolist()
n_imag4=table['吸收4（/cm）'].values.tolist()
n_imag5=table['吸收5（/cm）'].values.tolist()
n_imag=[]
for i in range(len(n_imag1)):
    n_imag.append(n_imag1[i])
for i in range(len(n_imag2)):
    n_imag.append(n_imag2[i])
for i in range(len(n_imag3)):
    n_imag.append(n_imag3[i])
for i in range(len(n_imag4)):
    n_imag.append(n_imag4[i])
for i in range(len(n_imag5)):
    n_imag.append(n_imag5[i])
for i in range(len(n_imag)):
    n_imag[i]=n_imag[i]*100*λ/(4*np.pi)
n_imag=[active_index_list1_ for active_index_list1_ in n_imag if active_index_list1_==active_index_list1_]
global n,d_m
n=[1+0j] #折射率，注意这个数组index=0时为空气，index最大时为衬底
for i in range(len(n_imag)):
    n.append(n_real[i]+n_imag[i]*1j)
d1=table['层厚1（nm）'].values.tolist() #每层厚度（从上往下）
d2=table['层厚2（nm）'].values.tolist() #每层厚度（从上往下）
d3=table['层厚3（nm）'].values.tolist() #每层厚度（从上往下）
d4=table['层厚4（nm）'].values.tolist() #每层厚度（从上往下）
d5=table['层厚5（nm）'].values.tolist() #每层厚度（从上往下）
d=[]
...
for i in range(len(d5)):
    d.append(d5[i])
d=[active_index_list1_ for active_index_list1_ in d if active_index_list1_==active_index_list1_]
d_m=d #约化为国际标准单位m
for i in range (len(d_m)):
    d_m[i]=d_m[i]/1E9
print(active_index,n_real,n_imag,d_m)
'''三层波导试探解'''
